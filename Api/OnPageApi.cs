...
namespace DataForSeo.Client.Api
{
    ...
    #pragma warning restore 8618
        ...
        public virtual async Task<OnPageUncrawlableResourcesResponseInfo> UncrawlableResourcesAsync(IEnumerable<OnPageUncrawlableResourcesRequestInfo> payload)
        {
            using (var request = new HttpRequestMessage())
            {
                var json = JsonConvert.SerializeObject(payload, _settings);
                ...
                if (!string.IsNullOrEmpty(_baseUrl))
                    urlBuilder.Append(_baseUrl);
                var path = "/v3/on_page/uncrawlable_resources";
                urlBuilder.Append(path);
                var url = urlBuilder.ToString();
                request.RequestUri = new Uri(url, UriKind.RelativeOrAbsolute);
                using (var response = await _httpClient.SendAsync(request, HttpCompletionOption.ResponseHeadersRead).ConfigureAwait(false))
                {
                    ...
                    if (status == 200)
                    {
                        var objectResponse = await ReadObjectResponseAsync<OnPageUncrawlableResourcesResponseInfo>(response, headers).ConfigureAwait(false);
                        return objectResponse.Object;
                    }
